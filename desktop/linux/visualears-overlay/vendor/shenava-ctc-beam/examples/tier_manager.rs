...
fn main() {
    ...
    let reg: Registry =
        serde_json::from_str(&std::fs::read_to_string(&path).expect("read registry"))
            .expect("parse registry");

    let ram = arg("--ram-mb")
        .and_then(|s| s.parse().ok())
        .unwrap_or_else(detect_ram_mb);
    let storage = arg("--storage-mb")
        .and_then(|s| s.parse().ok())
        .unwrap_or(u64::MAX / 2);
    let budget = arg("--budget-mb")
        .and_then(|s| s.parse().ok())
        .unwrap_or(u64::MAX / 2);

    let dev_mode = flag("--dev");
    println!(
        "device: {} MB RAM · {} storage · budget {}{}",
        ram,
        if storage == u64::MAX / 2 {
            "∞".into()
        } else {
            format!("{} MB", storage)
        },
    ...
    println!("\n  download plan:");
    let mut cache =
        std::path::PathBuf::from(arg("--cache").unwrap_or_else(|| "shenava_models".into()));
    cache.push(&p.id);
    ...
    if flag("--download") {
        ...
        for cname in &p.components {
            ...
            if ok && fname.ends_with(".zip") {
                Command::new("unzip")
                    .args(["-oq", out.to_str().unwrap(), "-d", cache.to_str().unwrap()])
                    .status()
                    .ok();
            }
